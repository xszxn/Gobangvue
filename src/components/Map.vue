<template>
  <div class="mapDiv">
    ...
    <div v-if ="reviewmode" class="button-group">
      <div
        class="button"
        @click="reviewcnt_back()"
      >后退一步</div>
      <div
        class="button"
        @click="reviewcnt_go()"
...
export default class Map extends Vue {
  ...
  gameAI(): void {
    ...
    if (!this.aihasdone && this.gameMode === 3 && this.whoDropping === Who.White) {
      let pos = this.calcPlacementCoordinate(
        this.matrix,
        PointColor.White,
        PointColor.White,
  ...
  reviewcnt_go(): void {
    if(this.reviewcnt < this.reviewlength)
      this.reviewcnt = this.reviewcnt + 1;
    else {
      alert("已经复盘到最后一步了");
      return;
  ...
  review_start(): void {
    ...
    this.stepStack = [];
    this.stepStack = this.stackcopy.slice(0, this.reviewcnt);
    for (let i = 0; i < this.stepStack.length; i++) {
      let [x, y, who] = this.stepStack[i];
      this.matrix[x][y] = who ? PointColor.Black : PointColor.White;
    }
    this.whoDropping = !this.stepStack[this.stepStack.length - 1][2];
    this.reviewmode = false;
    this.playing = true;
    this.drawChessboard(); 
  }
  ...
  drawChessboard(): void {
    ...
    if(this.reviewmode === false){
      ...
      if(this.gameMode === 4 && this.helpmode === true) {
        if(this.whoDropping === Who.White)
          this.canvasContext.fillStyle = "rgba(255,255,255,0.5)";
        else this.canvasContext.fillStyle = "rgba(0,0,0,0.5)";
        this.canvasContext.beginPath();
        this.canvasContext.arc(
          (this.aipositionx + 1) * this.latticeWidth,
          (this.aipositiony + 1) * this.latticeHeight,
          this.latticeWidth * 0.35,
          0,
          2 * Math.PI
        );
        this.canvasContext.closePath();
      ...
      if(this.gameMode === 3 && this.whoDropping === Who.White && this.stepStack.length > 1 && !this.aihasdone) {
        this.canvasContext.fillStyle = "rgba(255,255,255,0.5)";
        this.canvasContext.beginPath();
        this.canvasContext.arc(
          (this.aipositionx + 1) * this.latticeWidth,
      ...
      for (let i = 0; i < this.reviewcnt; i++) {
        let [x, y, who] = this.stackcopy[i];
        if (who) {
          //绘制一个黑子
          this.canvasContext.fillStyle = "#000000";
          this.canvasContext.beginPath();
